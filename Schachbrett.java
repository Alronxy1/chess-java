...
public class Schachbrett {
   ...
   static final String IMG = "U:/POSE/Schach/Chess_"; //Verändere den String-Inhalt je nachdem, wo die Bilder auffindbar sind (Datei-Pfad)
   ...
   public static void main(String[] args) {
      ...
      refreshBoard();
      System.out.println("WeiÃŸ ist am Zug. Bitte auf eine Figur klicken!");
   }
   ...
   public static void handleMouseClick(int r, int c) {
      ...
      if (selectedRow == -1 && selectedCol == -1) {
         ...
         if (!isValidMove(r1, c1, r, c, board, player)) {
            System.out.println("UngÃ¼ltiger Zug! Bitte erneut versuchen.");
            selectedRow = -1;
            selectedCol = -1;
            refreshBoard();
            return;
         ...
         // Flags vor dem Zug sichern
         boolean savedWKM = whiteKingMoved, savedBKM = blackKingMoved;
         boolean savedWRA = whiteRookAMoved, savedWRH = whiteRookHMoved;
         boolean savedBRA = blackRookAMoved, savedBRH = blackRookHMoved;
         ...
         if (kingPos == -1 || isAttacked(board, kingPos / 8, kingPos % 8, player)) {
            System.out.println("UngÃ¼ltiger Zug! Der eigene KÃ¶nig wÃ¤re im Schach.");
            board[r1][c1] = moved;
            board[r][c]   = captured;
         
            // Flags korrekt wiederherstellen
   ...
   public static boolean canCastle(int row, int kingCol, int targetCol) {
      ...
      if (player == 0) {
         if (whiteKingMoved) 
            return false;
         if (kingside && whiteRookHMoved) 
            return false;
         if (!kingside && whiteRookAMoved) 
            return false;
      } else {
         if (blackKingMoved) 
            return false;
         if (kingside && blackRookHMoved) 
            return false;
         if (!kingside && blackRookAMoved) 
            return false;
      }
      ...
      if (isAttacked(board, row, kingCol, player)) 
         return false;
      if (isAttacked(board, row, kingCol + step, player)) 
         return false;
      if (isAttacked(board, row, kingCol + 2 * step, player)) 
         return false;
   
      return true;
   }

   // Fährt die Rochade aus (König und Turm verschieben)
   public static void executeCastling(int r1, int c1, int r2, int c2) {
      ...
      System.out.println(castlePlayer + " rochiert " + castleSide);
   }
   ...
   public static boolean isValidMove(int row_own, int col_own, int row, int col, int[][] b, int player) {
      ...
      if (outOfBounds(row, col)) 
         return false;
      if (row_own == row && col_own == col) 
         return false;
   
      int piece = b[row_own][col_own];
      int piece_enemy = b[row][col];
   
      if (piece_enemy > player * 6 && piece_enemy <= player * 6 + 6) 
         return false;
      ...
      for (int i = 0; i < 64; i++) {
         ...
         if (piece > enemy_min && piece <= enemy_max) {
            if (piece == enemy_min + 6) {
               ...
               if (isValidMove(r, c, row, col, b, enemyPlayer)) 
                  return true;
            }
         }
      }
